...
from app.domain.enums import OverlayOp
...
class ResolvedRow:
    ...
    always comes straight from the underlying transaction), and the
    audit timestamps. INHERITED and OVERRIDDEN rows carry Base's own
    created_at/updated_at -- the overlay isn't a second, independently
    tracked lifecycle at this granularity (scenario_overlays has no
    ...
    def _resolve_rows(self, user_id: uuid.UUID, scenario: Scenario) -> list[ResolvedRow]:
        ...
        for txn in base_rows:
            ...
            if ov is None:
                ...
                out.append(
                    _row(txn, _patched_fields(txn, ov), EngineOrigin.OVERRIDDEN, scenario.id)
                )
...
def _row(
    txn: Transaction, fields: dict, origin: EngineOrigin, scenario_id: uuid.UUID
) -> ResolvedRow:
    resolved = ResolvedTransaction(
        id=str(txn.id),
