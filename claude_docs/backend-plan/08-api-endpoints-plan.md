...
| `DELETE /me` | Account deletion (screen-flow F9), distinct from `/me/data` reset below. Not in the architecture doc's locked §9 list — built as a small addition, see `12-open-questions-and-future-hardening.md` §3. **Soft delete** (product decision, §9 item 9): looks and behaves like a hard delete to the caller (can't log in again, every issued token invalid immediately, email free for a new registration right away), but the row and everything it owns physically survives for Phase 2's scheduled purge. | — |
...
| Endpoint | Purpose | Key error codes |
|---|---|---|
| `GET /scenarios/{id}/forecast?horizon=&anchor=&include_occurrences=` | The one payload that feeds the dashboard, all three forecast chart views, the monthly table, and month breakdown. `include_occurrences` (default `false`) adds each dated occurrence (name, direction, amount, date) behind `months[]` — the Home screen's "This Month" weekly chart needs day-level detail a monthly total can't give; see `12-open-questions-and-future-hardening.md` §9 item 10. | `forecast.invalid_horizon`, `resource.not_found` |
| `GET /forecast/compare?a=&b=&horizon=&anchor=` | Two ledgers aligned by month, plus deltas and the drivers list. Each driver carries `active_months` (distinct months it actually occurs in) so the client can compute its true per-occurrence rate (`total_contribution_minor / active_months`) instead of diluting it across the whole horizon — see `12-open-questions-and-future-hardening.md` §9 item 7. | `compare.same_scenario`, `scenario.archived` (either operand — architecture §6.2), `forecast.invalid_horizon`, `resource.not_found` |
