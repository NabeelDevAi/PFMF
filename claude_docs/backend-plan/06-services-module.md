...
| `ForecastService` | Resolving `anchor_month` (from `balance_as_of`, if not given explicitly — never the clock) and horizon, calling the resolver, calling the engine, shaping the ledger into an API response | The engine itself never reads a clock — this service is where "today" is read, exactly once per call, only to compute `current_month`/`months_elapsed` |
